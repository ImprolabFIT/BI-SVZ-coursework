...
** `conda create --n SVZ --file <cesta k SVZ-spec-file-win64.txt>`
...
[NOTE]
====
Nabízím 1 bod aktivity za doplnění instalace na macOS (64bit, popř. jiné chtěné). Včetně vytvoření souboru `SVZ-spec-file-osx64.txt` (viz https://conda.io/docs/user-guide/tasks/manage-environments.html#building-identical-conda-environments[návod]). Soubor `SVZ-pip-req.txt` by měl snad fungovat pro všechny systémy stejný jeden.
