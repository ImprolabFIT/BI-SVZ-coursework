...
Úkoly budou ve stejném formátu jako probíhá práce na cvičení, tzn. Jupyter notebooky. Na domácím PC tedy doporučujeme nainstalovat multiplatformní Anacondu pro správu Python balíčku a vytvořit si virtuální prostředí viz xref:../tutorials/files/1/course-tools-introduction#[návod]. Pokud nemáte systém Win x64 nebo MacOS, tak se vám pravděpodobně nepodaří vytvořit prostředí pomocí spec-file a budete muset balíčky nainstalovat ručně. Balíčky se instalují příkazem `conda install` viz xref:../tutorials/files/1/course-tools-introduction#[návod].
...
* link:../tutorials/files/1/env/SVZ-pip-req.txt[Seznam balíčků v txt]
...
Úkoly odevzdávejte na FIT Gitlab. Vytvořte si privátní repozitář a přidejte do něj uživatele `@zitnyjak`, `@brchlluk` a `@novakj67` s rolemi "reporter".
