...
Rozdíl mezi těmito konkrétními nástroji je markantní. Google Cloud Vision API funguje lépe, je postaven na moderních metodách hlubokých neuronových sítí, ale je dostupný pouze online přes API a po zřízení účtu na Cloud Vision. 
...
* Restartovat počítač (možná není nutné).
